<x-app-layout>
    <div class="max-w-2xl mx-auto p-16 mt-8 bg-gray-700 border rounded-lg">
        <div class="card-header text-white text-2xl">Editar Notícia</div>
        <div class="card-body">
            <form action="{{ route('noticia.update', $noticia->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="gap-6 mb-6 grid-cols-2 mt-9">
                    <div class="lg:w-[542px]">
                        <label for="title" class="block mb-2 text-sm font-medium text-white dark:text-gray-300">Nome da Notícia</label>
                        <input type="text" id="title" name="title" value="{{ old('name', $noticia->title) }}"
                    ...
                    <div class="lg:w-[542px]">
                        ...
                        <textarea id="description" name="description"
                                  ...
                                  placeholder="Descrição" required>{{ old('description', $noticia->description) }}</textarea>
                    </div>
                </div>
                <div>
                    <label for="image" class="mx-auto cursor-pointer flex w-full max-w-lg flex-col items-center rounded-xl border-2 border-dashed border-blue-400 bg-white p-6 text-center">
                ...
                <div>
                    <label for="entidade_id" class="block mb-2 text-sm font-medium text-gray-700">Local</label>
